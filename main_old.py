...
# settings of CORS midleware to allow API to access the HTML file opened by browser 
origins = [
    # add "null" to allow local file (file://) to acess
    "null",
    # Considering hosting on a web server in the future
    "http://localhost",
    "http://localhost:8080",
]
...
def get_note(exp_id: str):
    ...
    # Consider the case where no node was found or there are no flowchart nodes in the note
...
def list_experiments(project_name: str, folder_path: str = "/"):
    ...
    if not path_parts: # if it is the root folder
        query = """
            MATCH (p:Project {name: $p_name})-[:CONTAINS]->(e:Experiment)
        ...
        # step1: Define the route pattern of the node with MATCH
        # example: (p:Project)-[:CONTAINS]->(f0:Folder)-[:CONTAINS]->(f1:Folder)
        path_pattern = "(p:Project)"
        for i in range(len(path_parts)):
            path_pattern += f"-[:CONTAINS]->(f{i}:Folder)"
        path_pattern += "-[:CONTAINS]->(e:Experiment)"

        #step2: Define the conditions for each node in the WHERE clause
        # example: p.name = $p_name AND f0.name = $f_name_0 AND f1.name = $f_name_1
        ...
        #step3: Combine the above to generate the complete query
        query = f"""
            MATCH {path_pattern}
            WHERE {where_str}
            RETURN e.id as id, e.name as name, e.registrant as registrant, e.registration_date as registration_date
